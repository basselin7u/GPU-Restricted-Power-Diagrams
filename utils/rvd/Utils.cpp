...
namespace Utils {
static void getBoundingBox(const std::vector<float>& xyzw, float& xmin, float& ymin, float& zmin, float& xmax, float& ymax, float& zmax) {
 ...
 bool loadSeedsFile(const char* filename, std::vector<float>& xyzw, bool is4DFile, bool normalize) {
    std::ifstream in;
    ...
    if (normalize) { // normalize point cloud between [0,1000]^3
        ...
        float maxside = fmax(fmax(xmax-xmin, ymax-ymin), zmax-zmin);
...
bool readBinarySeedsFile(const char* filename, std::vector<float>& xyzw) {
    ...
    if (N==0) return false;
    xyzw.resize(4*N);
    file.read(reinterpret_cast<char *>(xyzw.data()), long(4*sizeof(float)*N));
    if (!file.good()) {
...
bool saveBinarySeedsFile(const char* filename, std::vector<float> const &xyzw) {
    ...
    file.write(reinterpret_cast<char const *>(xyzw.data()), long(4*sizeof(float)*N));
    if (!file.good()) {
...
bool readXYZGeogram(const char *filename, int &numVertices, std::map<std::string, std::vector<float> > &vertexData)
{
    ...
    while(!in.eof()) {
        ...
        vertexData[name]=std::vector<float>();
